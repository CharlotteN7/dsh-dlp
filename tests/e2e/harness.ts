...
/** File name of the plugin's audit sink inside the throwaway home. */
const AUDIT_LOG_NAME = 'dsh-dlp.audit.jsonl'

/** Placeholder substituted with the throwaway `$DSH_HOME` in task and argument text. */
...
export interface AgentRunOptions {
  /** The task string handed to `dsh --profile <name>`; `{{DSH_HOME}}` is substituted. */
  ...
  /** Raw JSON arguments emitted by every `tool_call_success` entry; `{{DSH_HOME}}` is substituted. */
 ...
 *
 * The plugin is copied, not symlinked, into the profile tree, so its own
 * pnpm store is out of Node's reach from there. Peer dependencies are
 * deliberately absent: every harness type this package uses is imported with
 * `import type`, so nothing from `@deepseek-ai/cordis` or the `dsh-*`
 * packages is emitted as a runtime import, and Cordis must come from the
 * running installation rather than a copy.
 ...
 * @param options - the task, the model script, seed files, and optional profile overrides.
 * @returns process output, audit records, the session log, and mock request records.
 */
export async function runAgent(options: AgentRunOptions): Promise<AgentRunResult> {
  ...
  try {
    ...
    server = await startMockLlmServer({
      ...
      ...options.toolArguments === undefined ? {} : { toolArguments: substitute(options.toolArguments) },
